// Vendored from FluxGarage RoboEyes 1.1.1, with two changes:
//
//   1. update() returns whether it drew a frame.
//   2. drawEyes() no longer calls display->display().
//
// Both exist so the deck can draw the slot strip into the same framebuffer as
// the face and flush once. A full SH1106 flush blocks for ~29.5ms, so a second
// one per frame would make the strip blink at the frame rate.
//
// Upstream: https://github.com/FluxGarage/RoboEyes
/*
...
// PATCHED: returns true when a frame was actually drawn, so the caller knows
// when to add its own overlay and flush. See the note at the top of the file.
bool update(){
...
void drawEyes(){
  ...
  // PATCHED: flush removed - the caller flushes after its overlay.
